...
from Objects import Stms, Program, SelectStatement, NumberExp, IdExp, BoolExp, BinaryExp
    ...
    def ParseStm(self):
        ...
        if self.match(Type.SELECT):
            ...
            while True:
                ...
                if self.match(Type.STAR) or self.match(Type.ID):
                    ...
                    raise ValueError("Se esperaba un identificador o * en SELECT.")

                if not self.match(Type.COMA):
                    raise ValueError("Se esperaba un ',' despues del ID en el SELECT")
            ...
            if not self.match(Type.INTO):
                raise ValueError("Se esperaba 'INTO' despues de 'INSERT'")
            ...
            if not self.match(Type.VALUES):
                raise ValueError("Se esperaba 'VALUES' despues del nombre de la tabla")

            if not self.match(Type.LPAREN):
                raise ValueError("Se esperaba '(' despues de 'VALUES'")

            values = []
            while True:
                if self.match(Type.STRING) or self.match(Type.NUMBER) or self.match(Type.TRUE) or self.match(
                        Type.FALSE):
            ...
            if not self.match(Type.RPAREN):
                raise ValueError("Se esperaba ')' despues de los valores")
